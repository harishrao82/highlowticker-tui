#!/usr/bin/env python3
"""
expire_backtest_no_cancel.py — Simulate expire_maker_no_cancel.py on historical data.

Per window, per symbol: resting YES+NO limit buys at EVERY cent from
LOW_LIMIT to HIGH_LIMIT. Both sides stay open — if both touch the price
level during the window, profit is LOCKED regardless of outcome.

Run:  python expire_backtest_no_cancel.py
"""
import sqlite3
...
def simulate_window(yes_arr, no_arr, winner):
    """
    No-cancel + replenish strategy:
    - Start with YES+NO resting at every price level in PRICES
    - When BOTH sides fill at a price (LOCK): add a new level one cent below the
      current lowest active level, simulate that level too
    - When only one side fills: single win/loss as normal, other side stays open
    - Replenish keeps going as long as new_p >= LOW_LIMIT and new_p not already active
    Returns total_pnl, fills list [(price, type, pnl), ...]
    """
    total_pnl = 0.0
    fills     = []

    # Active levels — starts as configured range, grows downward via replenish
    active = sorted(PRICES, reverse=True)  # high→low
    active_set = set(active)

    processed = set()  # avoid re-processing same price twice
    ...
    while i < len(active):
        ...
        if t_yes != 9999 and t_no != 9999:
            # LOCK — both sides filled
            pnl = round(1.0 - p * 2, 2)
            if pnl > 0:
                total_pnl += pnl
                fills.append((p, "LOCK", pnl))

                # Replenish: one cent below current lowest active
                lowest = min(active_set)
                new_p  = round(lowest - 0.01, 2)
                if new_p >= LOW_LIMIT and new_p not in active_set:
                    active.append(new_p)
                    active_set.add(new_p)

        elif t_yes != 9999:
            won = (winner == "yes")
            pnl = round(1.0 - p, 2) if won else round(-p, 2)
            total_pnl += pnl
            fills.append((p, "YES", pnl))
        else:
            won = (winner == "no")
            pnl = round(1.0 - p, 2) if won else round(-p, 2)
            total_pnl += pnl
            fills.append((p, "NO", pnl))
