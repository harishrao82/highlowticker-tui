            ...
            async with websockets.connect(
                ...
                async for raw in ws:
                    ...
                    if msg.get("channel") != "ticker":
                        continue
                    # Use the message's server timestamp if present, else local
                    ...
                    for ev in msg.get("events", []):
                        for t in ev.get("tickers", []):
                            ...
                            if sym and px > 0:
                                # Only record on actual change (avoid flooding)
                                if live_prices.get(sym) != px:
                                    asyncio.ensure_future(btc_db.record_spot_tick(
                                        sym, ts_ms, px, "coinbase",
                                    ))
                                live_prices[sym] = px
                                if sym == "BTC":
                                    _btc_price = px
            ...
            async with websockets.connect(
                ...
                async for raw in ws:
                    ...
                    if mtype == "ticker":
                        ...
                        if sym and ya > 0:
                            # Detect change to avoid redundant rows
                            prev_ya = state[sym]["yes_ask"]
                            prev_na = state[sym]["no_ask"]
                            state[sym]["yes_ask"] = ya
                            state[sym]["no_ask"]  = na
                            # Sub-second tick: only write on actual change
                            wid = state[sym].get("window_id", 0)
                            if wid > 0 and (ya != prev_ya or na != prev_na):
                                window_ts = state[sym].get("window_ts", 0)
                                if window_ts > 0:
                                    el_ms = int((time.time() - window_ts) * 1000)
                                    if 0 <= el_ms <= 900_000:
                                        asyncio.ensure_future(btc_db.record_tick_ms(
                                            wid, el_ms, ya, na,
                                        ))
...
async def _record_loop(client: httpx.AsyncClient) -> None:
    ...
    while True:
        ...
        # Per-coin display every 5 seconds — show both in-window Δ and
        # distance-to-strike (ITM), plus model odds for BTC.
        if e_int % 5 == 0:
            header = f"[dim]t={e_int:>3}s[/dim]"
            lines = [header]
            any_printed = False
            for sym in COINS:
                st  = state[sym]
                ya  = st["yes_ask"]
                na  = st["no_ask"]
                now_px  = live_prices.get(sym, 0.0)
                open_px = st.get("coin_open_price") or 0.0    # set in _init_windows
                floor   = st.get("floor") or 0.0
                if ya <= 0 or now_px <= 0:
                    continue
                # Δ  = in-window momentum (matches live trader's reference)
                if open_px > 0:
                    delta_pct = (now_px - open_px) / open_px * 100.0
                    d_col = "green" if delta_pct > 0 else "red" if delta_pct < 0 else "white"
                    delta_str = f"[{d_col}]Δ{delta_pct:+.3f}%[/{d_col}]"
                else:
                    delta_pct = None
                    delta_str = "[dim]Δ?[/dim]"

                # ITM = distance to Kalshi's settlement strike. Show "?" when
                # floor_strike hasn't been fetched yet (async REST lag at window start).
                if floor > 0:
                    itm_pct = (now_px - floor) / floor * 100.0
                    i_col = "green" if itm_pct > 0 else "red" if itm_pct < 0 else "dim"
                    itm_str = f"[dim]ITM[/dim][{i_col}]{itm_pct:+.3f}%[/{i_col}]"
                else:
                    itm_pct = None
                    itm_str = "[dim]ITM?[/dim]"

                model_str = ""
                if sym == "BTC" and itm_pct is not None:
                    # Model was trained on delta-vs-strike — feed it ITM
                    p_up, p_down, n_eff = _get_odds(e_int, itm_pct)
                    model_str = f"  [dim]model Y=[/dim]{p_up:.0%} [dim]N=[/dim]{p_down:.0%}"

                lines.append(
                    f"  {sym:3}  Y={ya:.2f} N={na:.2f}  "
                    f"{delta_str}  {itm_str}{model_str}"
                )
                any_printed = True
