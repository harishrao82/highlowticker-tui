#!/usr/bin/env python3
"""
expire_maker_simple.py

Strategy (original / back to basics):
  - Batch place YES + NO limit buys at every cent from low_limit → high_limit
  - Listen for fills via WebSocket — log only, no cancels, no rebalance
  - Every 60s: print status per coin (lowest fill, dominant side, counts)
  - End of window → cancel all remaining UNFILLED orders
  - Let filled positions expire and settle naturally

0% maker fee.

Run:   python expire_maker_simple.py
Stop:  Ctrl-C
"""
import asyncio
import base64
...
COINS = {
    "BTC": dict(enabled=True,  series="KXBTC15M", low_limit=0.02, high_limit=0.25, shares=1),
    "ETH": dict(enabled=True,  series="KXETH15M", low_limit=0.02, high_limit=0.33, shares=1),
    "SOL": dict(enabled=True,  series="KXSOL15M", low_limit=0.02, high_limit=0.35, shares=1),
    "XRP": dict(enabled=True,  series="KXXRP15M", low_limit=0.02, high_limit=0.25, shares=1),
}

# Hard cutoff: cancel every unfilled order this many seconds into the window.
# Filled positions still ride to settlement.
CANCEL_AT_SECONDS = 300   # 5 minutes

RUN_WINDOWS = 16  # 0 = run forever
...
async def _cancel_batch(
    client: httpx.AsyncClient, oids: list[str], tag: str,
...
async def run_window(
    ...
    # ── Pre-generate coids so fills during placement are never missed ─────────
    coid_map: dict[str, tuple[str, float, str]] = {}
    ...
    try:
        async with websockets.connect(
            ...
            # ── Every 15s: compact fills array per coin ──────────────────
            async def status_loop() -> None:
                while True:
                    await asyncio.sleep(15)
                    ts_s      = datetime.now().strftime("%H:%M:%S")
                    remaining = deadline - asyncio.get_event_loop().time()
                    for sym_s, st_s in states.items():
                        filled: list[tuple[float, str]] = []
                        for price, lv_ in st_s.levels.items():
                            if lv_.get("yes_filled"):
                                filled.append((price, "Y"))
                            if lv_.get("no_filled"):
                                filled.append((price, "N"))
                        filled.sort(key=lambda x: x[0], reverse=True)
                        parts = " ".join(
                            f"{int(round(p*100))}-{s}" for p, s in filled
                        )
                        console.print(
                            f"  [dim][{ts_s}] {sym_s}:[{parts}]"
            ...
            deadline      = window_start + 895
            cutoff_time   = window_start + CANCEL_AT_SECONDS
            cutoff_done   = False
            next_tickers: dict[str, tuple[str, datetime]] = {}
            prefetch_done = False

            while asyncio.get_event_loop().time() < deadline:
                if not cutoff_done and asyncio.get_event_loop().time() >= cutoff_time:
                    cutoff_done = True
                    ts_cut = datetime.now().strftime("%H:%M:%S")
                    console.print(
                        f"\n  [bold yellow][{ts_cut}] Cutoff reached"
                        f" ({CANCEL_AT_SECONDS}s) — cancelling unfilled orders"
                        f" (highest price first)[/bold yellow]"
                    )
                    # Sort highest-price-first: riskier singles (bigger -p loss)
                    # get cancelled before cheaper ones, so any in-flight fills
                    # during the cancel phase land on safer levels.
                    unfilled_with_price: list[tuple[float, str]] = []
                    for st in states.values():
                        for price, lv in st.levels.items():
                            for side in ("yes", "no"):
                                if not lv.get(f"{side}_filled") and lv.get(side):
                                    unfilled_with_price.append((price, lv[side]))
                    unfilled_with_price.sort(key=lambda x: x[0], reverse=True)
                    unfilled_now = [oid for _, oid in unfilled_with_price]
                    if unfilled_now:
                        await _cancel_batch(client, unfilled_now, "cutoff", batch_sleep=2.0)
                    else:
                ...
                if already_filled:
                    console.print(
                        f"  [bold yellow][{ts}] {sym}@{price:.2f}  LOCK"
                        f"  +${1.0 - price*2:.2f} guaranteed[/bold yellow]"
                    )
                else:
                    console.print(
                        f"  [bold green][{ts}] {sym}@{price:.2f}  {side.upper()}"
        ...
        console.print("\n  [yellow]Interrupted — cancelling unfilled orders…[/yellow]")
        unfilled = [
            lv[side]
            for st in states.values()
    ...
    # ── End-of-window: cancel all unfilled, let filled positions expire ───────
    unfilled = [
        lv[side]
        for st in states.values()
        for lv in st.levels.values()
...
async def main() -> None:
    active = {sym: cfg for sym, cfg in COINS.items() if cfg["enabled"]}
    console.print("[bold cyan]Expire Maker Simple[/bold cyan]")
    console.print(
        f"Strategy: place YES+NO at every level, cancel unfilled at "
        f"{CANCEL_AT_SECONDS}s ({CANCEL_AT_SECONDS//60}m), let filled positions expire\n"
    )
    for sym, cfg in active.items():
