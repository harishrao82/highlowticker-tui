...
SIZE_RATIO = 80   # Agile trader uses 80:1 (half Lively's exposure)

LIVELY_SIGNALS = Path.home() / ".agile_fast_signals.jsonl"
LIVE_TRADES    = Path.home() / ".kalshi_agile_trades.jsonl"
SHADOW_TRADES  = Path.home() / ".agile_shadow_trades.jsonl"   # unused fallback
STATUS_FILE    = Path.home() / ".kalshi_agile_status.json"
...
async def fetch_loop() -> None:
    ...
    while True:
        state["syncing"] = True
        t0 = time.time()
        ok1 = await _scp_one("~/.kalshi_agile_trades.jsonl",   LIVE_TRADES)
        ok2 = await _scp_one("~/.agile_fast_signals.jsonl",    LIVELY_SIGNALS)
        await _scp_one("~/.kalshi_agile_status.json",          STATUS_FILE)
        dur = time.time() - t0
        state["syncing"] = False
...
def build_header(now: int) -> Panel:
    ...
    line1 = (f"[bold cyan]✦ Agile Monitor[/]   "
