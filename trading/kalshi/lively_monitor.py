...
SIZE_RATIO = 40   # we trade 1 share per N of his — multiply our P&L by this for apples-to-apples edge

LIVELY_SIGNALS = Path.home() / ".lively_fast_signals.jsonl"
LIVE_TRADES    = Path.home() / ".kalshi_lively_trades.jsonl"
SHADOW_TRADES  = Path.home() / ".lively_shadow_trades.jsonl"
STATUS_FILE    = Path.home() / ".kalshi_lively_status.json"   # written by trader, scp'd here
...
async def fetch_loop() -> None:
    ...
    while True:
        state["syncing"] = True
        t0 = time.time()
        ok1 = await _scp_one("~/.kalshi_lively_trades.jsonl", LIVE_TRADES)
        ok2 = await _scp_one("~/.lively_fast_signals.jsonl",   LIVELY_SIGNALS)
        await _scp_one("~/.kalshi_lively_status.json",         STATUS_FILE)  # canonical his_pos from trader
        dur = time.time() - t0
        state["syncing"] = False
...
def build_header(now: int) -> Panel:
    ...
    line1 = (f"[bold magenta]✦ Lively Monitor[/]   "
